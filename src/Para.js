...
const Para = () =>{
  ...
  I just told you! You've killed me! Fry! Quit doing the right thing, you jerk! Michelle, I don't regret this, but I both rue and lament it. Morbo can't understand his teleprompter because he forgot how you say that letter that's shaped like a man wearing a hat.`,
    ...
    function handleClick(value){
        ...
        const item = Paras.slice(0,value);
        setArr(item);
    }
    }
    return(
        ...
        <p> Paragraphs <input type="number" min={-10} max={8} className="border border-black rounded" onChange={(e)=> setValue(e.target.value)} /></p>
        <button className="border border-black rounded bg-sky-400 px-2"
            onClick={() => handleClick(value)}
        >Generate</button>
        < div className="and">
        <div className="paras">
            {arr.map((para,index)=> <p key={index}>{para}</p> )}
        </div></div>
        </div>
        </div>
    )
